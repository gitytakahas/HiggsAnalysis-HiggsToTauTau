...
//#include "/afs/cern.ch/work/y/ytakahas/H2TauTau_bbb_pT30/CMSSW_6_1_1/src/HiggsAnalysis/HiggsToTauTau/src/HttStyles.cc"
...
{  
  ...
  for(Int_t f=0;f<n;f++){
    ...
    if((Input[f].Contains("emu") && SM) ){
      ggH = (TH1F*)F.Get("ggH_hww")->Clone("ggH");
      //ggH = (TH1F*)F.Get("ggH")->Clone("ggH");
    }else if( (Input[f].Contains("eleTau_0jet") && SM) || (Input[f].Contains("muTau_0jet") && SM) || (Input[f].Contains("ee") && SM) || (Input[f].Contains("mumu") && SM) ){
      ggH = (TH1F*)(F.Get("Ztt")->Clone("ggH"));
      
      for(int ibin=1; ibin<=ggH->GetXaxis()->GetNbins(); ibin++){
	ggH->SetBinContent(ibin,0);
	ggH->SetBinError(ibin,0);
      }
    ...
    if(ggH==0){
      //      ggH = (TH1F*)(F.Get("Ztt")->Clone("ggH"));
      std::cout << "ggH is missing " << f << "/" << n << std::endl;
      //      continue;
    }else if(Ztt==0){
      std::cout << "Ztt is missing " << f << "/" << n << std::endl;
      //      continue;
    }
  ...
  cout<<" Ordered Weights for " << n<<endl;
  float maxw=100000.0;
  ...
  for(Int_t f=0;f<n;f++){
    ...
    if((Input[f].Contains("emu") && SM) || (Input[f].Contains("ee") && SM) || (Input[f].Contains("mumu") && SM)){
      ggH=(TH1F*)File.Get("ggH_hww")->Clone("ggH");
      //ggH=(TH1F*)File.Get("ggH")->Clone("ggH");
      //    }else if( (Input[f].Contains("eleTau_0jet") && SM) || (Input[f].Contains("muTau_0jet") && SM) ){
    ...
    if(!ggH) std::cout << " ggH is missing for " << Input[f].Data()<<endl;
    if(!Ztt) std::cout << " Ztt is missing for " << Input[f].Data()<<endl;
  ...
  //  if((Input[0].Contains("emu") && SM) || (Input[0].Contains("ee") && SM) || (Input[0].Contains("mumu") && SM)){
  //  if( (Input[f].Contains("eleTau_0jet") && SM) || (Input[f].Contains("muTau_0jet") && SM) || (Input[f].Contains("ee") && SM) || (Input[f].Contains("mumu") && SM) ){
  if(Input[0].Contains("emu") && SM){
    ggH1=(TH1F*)F1.Get("ggH_hww");
    //ggH1=(TH1F*)F1.Get("ggH");
  }else if( (Input[f].Contains("eleTau_0jet") && SM) || (Input[f].Contains("muTau_0jet") && SM) || (Input[f].Contains("ee") && SM) || (Input[f].Contains("mumu") && SM)){
    ggH1 = (TH1F*)(F.Get("Ztt")->Clone("ggH"));
    for(int ibin=1; ibin<=ggH->GetXaxis()->GetNbins(); ibin++){
      ggH1->SetBinContent(ibin,0);
      ggH1->SetBinError(ibin,0);
    }
  }
...
//
//    if(Input[0].Contains("emu")  && SM){
//      std::cout << "Yuta tes12 " << F2.Print() << std::endl;
//      ggH2=(TH1F*)F2.Get("ggH_hww")->Clone("ggH2");
//      std::cout << "Yuta tes12-2 " << F2.Print() << std::endl;
//
//    }else if( (Input[f].Contains("eleTau_0jet") && SM) || (Input[f].Contains("muTau_0jet") && SM) ){
//      std::cout << "Yuta tes13 " << F2.Print() << std::endl;
//      ggH2 = (TH1F*)(F.Get("Ztt")->Clone("ggH2"));
//      for(int ibin=1; ibin<=ggH->GetXaxis()->GetNbins(); ibin++){
//	ggH2->SetBinContent(ibin,0);
//	ggH2->SetBinError(ibin,0);
//      }
//    }else{
//      std::cout << "Yuta tes14 " << F2.Print() <<  std::endl;
//      ggH2 = (TH1F*)(F2.Get("ggH")->Clone("ggH2"));
//    }
//    
//    std::cout << "Yuta tes14 " << F2.Print() << std::endl;
//
...
//    }
//    
//    std::cout << "Yuta tes15 " << F2.Print() << std::endl;
//
    ...
    TH1F* ggH_hww2;
    if(Input[f].Contains("emu") && SM){
      ggH_hww2=(TH1F*)(F2.Get("ggH_hww")->Clone("ggH_hww2"));
    }
  ...
  std::cout << "Yuta : " << outname << std::endl;
